...
// 	const handleCategoryChange = (categoryId, name) => {
// 		console.log('handleCategoryChange called with:', categoryId, name);
// 		const queryParams = new URLSearchParams(location.search);
// 		if (categoryId === "") {
	...
	useEffect(() => {
		async function fetchCategories() {
			try {
				...
				if (parCategoryName) {
					const matchedCategory = response.data.categories.find(
						(category) => category._id === parCategoryName
					);
					if (matchedCategory) {
						setSelectedCategory(matchedCategory._id);
						setCategoryName(matchedCategory.name);
		...
		<>
			<div className="main_product_sidebar_top_parent_div">
				<div className="main_product_sidebar_div">
					{/* Product Search */}
					<div className="product_sliderbar_options">
						<div className="proudct_sidebar_heading product-heading1">Product Search:</div>
						<div>
							<input
								type="text"
								placeholder="Search"
